...
# O nome da funcao foi mantido por compatibilidade com o projeto atual,
# mas a regra aplicada agora e a de menor tempo de vida.
def schedule_priority(processes: Sequence[Process]) -> ScheduleResult:
    return _schedule_non_preemptive(
        processes=processes,
        algorithm_name="PS adaptado (menor tempo de vida)",
        candidate_key=lambda state: (
            state.current_life_time,
            state.process.arrival_time,
            state.process.burst_time,
        ...
        selection_reason=(
            "menor tempo de vida atual; adaptacao hospitalar do antigo PS"
        ),
    )
